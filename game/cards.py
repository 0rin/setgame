...
class Cards(object):
    ...
    @classmethod
    def initialize_new_game(cls):
        Cards.deck = Deck().new_shuffled_deck()
        Cards.number_sets_found = 0
        Cards.cards_open = Cards._take_n_cards(12)
        # Cards.cards_open = Cards.setless
        # Cards.cards_open = Cards.cards_18
        Cards.a_set = False
    ...
    def open_extra_cards(cls):
        ...
        does not change.
        """
        Cards.indices_of_extra_cards = Cards._find_indices_of_extra_cards()
        for i in Cards.indices_of_extra_cards:
            Cards.cards_open.insert(i, Cards._take_n_cards(1)[0])


    @classmethod
    def check_for_set(cls):
        """
        ...
        for combo in combinations(Cards.cards_open, 3):
            ...
            if blank:
                continue
            elif Cards._validate_set(combo):
                Cards.a_set = combo[0]
            ...
            for j, set_card_id in enumerate(cards_in_set):
                if card['id'] == int(set_card_id):
                    if extra_cards_open and i in extra_cards:
                        ...
                        Cards.cards_open[i] = Cards._take_n_cards(1)[0]
                    del cards_in_set[j]

        if extra_cards_open:
            Cards._handle_extra_open_cards(extra_cards, to_replace)


    @classmethod
    def _handle_extra_open_cards(cls, extra_cards, to_replace):
    ...
    @classmethod
    def _find_indices_of_extra_cards(cls):
        result = []
    ...
    @classmethod
    def _validate_set(cls, combo):
        colors = numbers = shadings = shapes = []
